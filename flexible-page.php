...
if( class_exists('acf') ) { 
    if ( get_field('flex_full_width') ) {
        ...
        add_action( 'genesis_entry_content', __NAMESPACE__. '\replace_header_with_wrap', 1 );
        // Place Flexible content
        add_action( 'genesis_entry_content', __NAMESPACE__. '\add_flexible_content', 1 );
        // Place content with wrap
        add_action( 'genesis_entry_content', __NAMESPACE__. '\replace_content_with_wrap', 1 );
    } else {
        // Place Header with hero options
        add_action( 'genesis_entry_content', __NAMESPACE__. '\replace_header', 1 );
        // Place Flexible content
        add_action( 'genesis_entry_content', __NAMESPACE__. '\add_flexible_content', 1 );
        // Place content
        add_action( 'genesis_entry_content', __NAMESPACE__. '\replace_content', 1 );
    }
} else {
    // Place content
    add_action( 'genesis_entry_content', __NAMESPACE__. '\replace_content', 1 );
